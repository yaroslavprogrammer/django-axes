...
def _get_user_attempts(request):
    ...
    if USE_USER_AGENT:
        ua = request.META.get('HTTP_USER_AGENT', unicode(_('<unknown>')))[:255]
    ...
    def decorated_login(request, *args, **kwargs):
        ...
        if request.method == 'POST':
            ...
            access_log = AccessLog.objects.create(
                user_agent=request.META.get('HTTP_USER_AGENT', unicode(_('<unknown>')))[:255],
                ip_address=get_ip(request),
                username=request.POST.get('username', None),
                http_accept=request.META.get('HTTP_ACCEPT', unicode(_('<unknown>'))),
                path_info=request.META.get('PATH_INFO', unicode(_('<unknown>'))),
...
def lockout_response(request):
    ...
    if COOLOFF_TIME:
        return HttpResponse(_("Account locked: too many login attempts. "
    ...
    if login_unsuccessful:
        ...
        if len(attempts):
            for attempt in attempts:
                ...
                attempt.http_accept = request.META.get('HTTP_ACCEPT', unicode(_('<unknown>')))
                attempt.path_info = request.META.get('PATH_INFO', unicode(_('<unknown>')))
